...
public class EntityLifeCycleTests {
    ...
    void testTransient(int menuCode) {
        ...
        assertTrue(entityManager.contains(foundMenu));      // foundMenu는 영속석 컨텍스트에서 관리되는 영속 상태의 객체
        assertFalse(entityManager.contains(newMenu));        // newMenu는 영속성 컨텍스트에서 관리 되지 않는 비영속 상ㄴ태의 객체
    }
    ...
    @ParameterizedTest
    @ValueSource(ints = {2, 3})
    void testManagerSameEntityManager(int menuCode) {
    ...
    void testDetachEntity(int menuCode, int menuPrice) {
        ...
        // flush : 영속석 컨텍스트의 상태를 DB로 내보낸다. commit 하지 않은 상태이므로 rollback이 가능하다.
        entityManager.flush();
    ...
    void testRemoveEntity(int menuCode) {
        ...
        // 단, 트랜젝션을 제어하지 않으면 데이터 베이스에서 영구 반영 되지는 않는다.
        // 트랜젝션을 커밋하는 순간 영속성 컨텍스트에서 관리하는 엔티티 객체가 데이터 베이스에 반영 된다.
